...
export class MessageModerationService {
  ...
  public async validateMessages() {
    await this.redlock.using(
      [`forum:messageValidationTask`],
      10000,
      async (signal) => {
        const batch = await this.messageEntityRepository.find({
          where: {
            created_at: MoreThan(await this.getCutoffDate()),
            deleted: false,
        ...
        await this.setCutoffDate(new Date());
      },
    );
  }

  private async setCutoffDate(date: Date) {
    await this.redis.set(
      'forum:messageValidationTaskCutoffDate',
      date.toISOString(),
    );
  }

  private async getCutoffDate(): Promise<Date> {
    const d: string = await this.redis.get(
      'forum:messageValidationTaskCutoffDate',
    );
    if (!d) {
      return new Date();
